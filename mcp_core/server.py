...
class BaseMCPServer:
    ...
    async def handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        if len(self.clients) >= self.config.max_connections:
            logger.warning(f"Connection rejected - max connections reached: {client_ip}")
            writer.close()
            await writer.wait_closed()
            return
    ...
    async def _handle_client_communication(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter, session: ClientSession):
        ...
        raise NotImplementedError("Subclasses must implement _handle_client_communication")
